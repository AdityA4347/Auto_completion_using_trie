...
public:
    ...
    ~node() {}
    char Data() //these are the data members 
    {
        return wData;
    }
    void setData(char c)
    ...
    void setWordTerminate()//member functions makes the last terminal node as true
    {
        wEnd = true;
    }
    ...
    {
        wChildren.push_back(child);// insert char from end
    }
...
private:
    char wData;
    bool wEnd; //to tell that the word ends
    vector<node*> wChildren;
};

node* node::findingChild(char c) //finds the char whether it is in trie or not
{
...
private:
    node* root; //first we create basic root node 
};
Trie::Trie()
{
    root = new node();
...
{
    node* current = root;
    if ( s.length() == 0 ) //chect word length must not be zero
    {
        current->setWordTerminate(); // an empty word set the last character as terminal node
        return;
    }
    for ( int ii = 0; ii < s.length(); ii++ )
    {
        node* child = current->findingChild(s[ii]);
        //the word is present already
        if ( child != NULL )
        {
            current = child;
        }
        ...
        {
            node* tmp = new node(); //so create new node
            tmp->setData(s[ii]);
